...
impl RollingFileAppender {
    ...
    fn from_builder(builder: &Builder, directory: impl AsRef<Path>) -> Result<Self, InitError> {
        ...
        let now = OffsetDateTime::now_utc().to_offset(
            clia_local_offset::current_local_offset().expect("Can not get local offset!"),
        );
...
impl Inner {
    fn new(
        ...
        let symlink_path = Path::new(&log_directory).join(
            &log_filename_prefix
                .clone()
                .unwrap_or("nofilename".to_owned()),
        );
        ...
        match create_writer(&self.log_directory, &filename) {
            Ok(new_file) => {
                ...
                *file = new_file;
                ...
                // Create a symlink to latest log file.
                let latest_path = Path::new(&filename);
                let symlink_path = Path::new(&self.log_directory).join(
                    &self
                        .log_filename_prefix
                        .clone()
                        .unwrap_or("nofilename".to_owned()),
